...
| `frontend` | frontend work exists | Read plan + stack. Follow rest-api-design skill. Ask `backend` for API contracts if not in plan. **If stack is Angular:** follow the `angular-components`, `angular-material`, and `design-md` skills from stitch-skills. All components must be standalone, OnPush, use signals (`input()`, `output()`, `signal()`, `computed()`), `inject()` for DI, and new control flow (`@for`, `@if`). Use Angular Material M3 with `mat.define-theme()`. Register routes lazily via `loadComponent()` in `app.routes.ts`. Verify each page visually using Playwright MCP before marking task complete. | components, pages, state, API integration, routing |
